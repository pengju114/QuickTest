...
public final class RestURISQLBuilder {
    ...
    static {
        ...
        sqlGenerators.put(RestURIPart.OPERATION_LIST, new ListSQLGenerator());
    ...
    public Tuple<CharSequence> build() throws Exception{
        if (restURIParts.size() < 1) {
            throw new IllegalArgumentException("no restURIParts");
        }
        StringBuilder sb = new StringBuilder();
        Tuple<CharSequence> result = new Tuple<CharSequence>();
        result.keyObject = sb;
        
        RestURIPart lastPart = restURIParts.getLast();
        
        OperationSQLGenerator sqlGenerator = OPERATION_SQL_GENERATORS.get(lastPart.getOperation());
        Tuple<CharSequence> t1 = sqlGenerator.generateSQL(this, lastPart);
        sb.append(t1.keyObject);
        result.valueObject.addAll(t1.valueObject);

        if (lastPart.getCondition() != null && lastPart.getCondition().isValid()) {
            Tuple<StringBuilder> cdt = ConditionStringGenerator.generateConditionSQL(this, lastPart);
            sb.append(" where ").append(cdt.keyObject);
    ...
    CharSequence createSelectSqlByURIPart(RestURIPart restURIPart) {
        ...
        if (impl.getSelectVars().size() > 0) {
            for (String var : impl.getSelectVars()) {
                builder.append(restURIPart.getPathAlias()).append('.');
        ...
        String table = evaluatePath(restURIPart.getPath());
        builder.append(" from ").append(table).append(" as ").append(restURIPart.getPathAlias());
    ...
    CharSequence createSelectSqlByVars(RestURIPart restURIPart, String... vars) {
        ...
        for (String var : vars) {
            ...
            builder.append(restURIPart.getPathAlias()).append('.').append(name).append(',');
        }
    ...
    private String evaluatePath(String path) {
        ...
        while (iterator.hasNext()) {
            ...
            String t = next.evaluatePath(this, path);
    ...
    public static interface RestURIBuilderDataSource {
    ...
    private static class CamelCaseDataSource implements RestURIBuilderDataSource {
        ...
        public String evaluatePath(RestURISQLBuilder restURISQLBuilder, String path) {
            ...
            return "t_" + evaluateVarName(restURISQLBuilder, path);
        }
    ...
    public static class Tuple<K> {

        public K keyObject;
        public final ArrayList<Object> valueObject = new ArrayList<Object>(3);
    }
    ...
    private static abstract class ConditionStringGenerator {
        ...
        public static Tuple<StringBuilder> generateConditionSQL(RestURISQLBuilder builder, RestURIPart uriPart) throws Exception{
            RestURICondition condition = uriPart.getCondition();
            if (!condition.isValid()) {
                return null;
            }

            Tuple<StringBuilder> tuple = new Tuple<StringBuilder>();
            ...
            while (currentPart != null) {
                ...
                Tuple<CharSequence> currentTuple = currentGenerator.conditionString(builder, uriPart, currentPart);
         ...
         * @throws Exception
         */
        public abstract Tuple<CharSequence> conditionString(RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart) throws Exception;
    ...
    private static abstract class CommonConditionStringGenerator extends ConditionStringGenerator {
        ...
        @Override
        public Tuple<CharSequence> conditionString(RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart) {
            StringBuilder builder = new StringBuilder();

            Tuple<CharSequence> tuple = new Tuple<CharSequence>();
            // 加上表的别名前缀,update 和 delete不能有别名
            if (isRequirePathAlias(uriPart)) {
                builder.append(uriPart.getPathAlias()).append('.');
            }
         ...
         * @param conditionPart
         */
        public void appendConditionPattern(StringBuilder stringBuilder, Tuple<CharSequence> tuple, RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart) {
        ...
        public Tuple<CharSequence> generateQuerySQL(RestURISQLBuilder builder, RestURIPart uriPart, RestURICondition.Part.Param param){
            RestURIPartImpl impl = (RestURIPartImpl) uriPart;
            impl.getSelectVars().add(param.getParam());
            Tuple<CharSequence> parentTuple = OPERATION_SQL_GENERATORS.get(impl.getOperation()).generateSQL(builder, impl);
            return parentTuple;
        }
         ...
         * @param param
         */
        public void appendParentConditionPattern(StringBuilder stringBuilder, Tuple<CharSequence> tuple, RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart, RestURICondition.Part.Param param) {
            Tuple<CharSequence> parentTuple = generateQuerySQL(sqlBuilder, uriPart.getParent(), param);
         ...
         * @param conditionPart
         */
        public void appendParamValue(Tuple<CharSequence> tuple, RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (param.getParamType() == RestURICondition.Part.Param.TYPE_VAR) {
                    ...
                    if (isRequirePathAlias(uriPart.getParent())) {
                        stringBuilder.append(uriPart.getParent().getPathAlias()).append('.');
                    }
    ...
    private static class InConditionStringGenerator extends CommonConditionStringGenerator {
        ...
        public void appendConditionPattern(StringBuilder stringBuilder, Tuple<CharSequence> tuple, RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart) {
            ...
            if (isVar) {
                ...
                throw new UnsupportedOperationException("Unsupported for varParam in");
            } else {
                for (RestURICondition.Part.Param param : params) {
                    stringBuilder.append('?').append(',');
                    tuple.valueObject.add(param.getParamValue());
    ...
    private static class OrderConditionStringGenerator extends CommonConditionStringGenerator {
        ...
        @Override
        public Tuple<CharSequence> conditionString(RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart) {
            if (!uriPart.getOperation().equalsIgnoreCase(RestURIPart.OPERATION_LIST) && !uriPart.getOperation().equalsIgnoreCase(RestURIPart.OPERATION_GET)) {
                throw new IllegalArgumentException("illegal condition:order ; order condition can only use for get and list operation");
            }
            StringBuilder builder = new StringBuilder();
        ...
        public void appendConditionPattern(StringBuilder stringBuilder, Tuple<CharSequence> tuple, RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart) {
            ...
            stringBuilder.append(uriPart.getPathAlias()).append('.');
    ...
    private static class IdConditionStringGenerator extends CommonConditionStringGenerator {
        ...
        public Tuple<CharSequence> conditionString(RestURISQLBuilder sqlBuilder, RestURIPart uriPart, RestURICondition.Part conditionPart) {
            ...
            // 加上表的别名前缀
            builder.append(uriPart.getPathAlias()).append('.');
            String id = sqlBuilder.evaluateVar(uriPart.getPath());
    ...
    private static abstract class OperationSQLGenerator {
        ...
        public abstract Tuple<CharSequence> generateSQL(RestURISQLBuilder builder, RestURIPart restURIPart);
    }

    private static class AddSQLGenerator extends OperationSQLGenerator {

        @Override
        public Tuple<CharSequence> generateSQL(RestURISQLBuilder builder, RestURIPart restURIPart) {
            StringBuilder stringBuilder = new StringBuilder(30);
            stringBuilder.append("insert into ");
            stringBuilder.append(builder.evaluatePath(restURIPart.getPath()));
    ...
    private static class ListSQLGenerator extends OperationSQLGenerator {

        @Override
        public Tuple<CharSequence> generateSQL(RestURISQLBuilder builder, RestURIPart restURIPart) {
            StringBuilder stringBuilder = new StringBuilder(20);
            stringBuilder.append("select ");
            stringBuilder.append(restURIPart.getPathAlias()).append('.').append('*');
            stringBuilder.append(" from ").append(builder.evaluatePath(restURIPart.getPath())).append(" as ").append(restURIPart.getPathAlias());
            Tuple<CharSequence> tuple = new Tuple<CharSequence>();
            tuple.keyObject = stringBuilder;
    ...
    private static class GetSQLGenerator extends ListSQLGenerator {
    ...
    private static class UpdateSQLGenerator extends OperationSQLGenerator {
        ...
        public Tuple<CharSequence> generateSQL(RestURISQLBuilder builder, RestURIPart restURIPart) {
            StringBuilder stringBuilder = new StringBuilder(30);
            stringBuilder.append("update ").append(builder.evaluatePath(restURIPart.getPath()));
    ...
    private static class DeleteSQLGenerator extends OperationSQLGenerator {
        ...
        public Tuple<CharSequence> generateSQL(RestURISQLBuilder builder, RestURIPart restURIPart) {
            ...
            stringBuilder.append("delete from ").append(builder.evaluatePath(restURIPart.getPath()));
